...
// Iluminação
void initLighting() {
    glEnable(GL_LIGHTING);                  // liga pipeline de iluminação
    glEnable(GL_LIGHT0);                    // habilita a luz 0 (o "Sol" na cena)

    // Ambiente global (iluminação de base da cena)
    GLfloat globalAmbient[] = {0.3f, 0.3f, 0.3f, 1.0f};
    glLightModelfv(GL_LIGHT_MODEL_AMBIENT, globalAmbient);

    // Espectro do Sol (luz branca com componente difusa e especular)
    GLfloat lightDiffuse[]  = {1.0f, 1.0f, 1.0f, 1.0f};
    GLfloat lightSpecular[] = {1.0f, 1.0f, 1.0f, 1.0f};
    glLightfv(GL_LIGHT0, GL_DIFFUSE,  lightDiffuse);
    glLightfv(GL_LIGHT0, GL_SPECULAR, lightSpecular);

    // Atenuação: controla queda de intensidade com a distância (ajustada para sua escala)
...
void display() {
    ...
    // Sol 
    glDisable(GL_LIGHTING);                             // o Sol é desenhado sem sombreamento
    glEnable(GL_TEXTURE_2D);                            // ativa texturização
    glBindTexture(GL_TEXTURE_2D, sunTexture);           // usa a textura do Sol
    ...
    for(int i=0; i<8; i++){
        ...
        glPushMatrix();
            ...
            GLfloat matDiffuse[]  = {1.0f, 1.0f, 1.0f, 1.0f}; // reflete cor da textura
            GLfloat matAmbient[]  = {1.0f, 1.0f, 1.0f, 1.0f}; // luz ambiente (levanta sombras)
            GLfloat matSpecular[] = {0.8f, 0.8f, 0.8f, 1.0f}; // brilho especular sutil
            glMaterialfv(GL_FRONT, GL_AMBIENT,  matAmbient);
            glMaterialfv(GL_FRONT, GL_DIFFUSE,  matDiffuse);
            glMaterialfv(GL_FRONT, GL_SPECULAR, matSpecular);
            glMaterialf (GL_FRONT, GL_SHININESS, 30.0f);       // tamanho do highlight
